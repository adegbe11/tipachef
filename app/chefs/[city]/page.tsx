...
import { CITIES, CHEF_CARDS } from "@/lib/pseo-data";
...
export async function generateMetadata(
  ...
  if (!data) return {};
  return {
    title:       `Support ${data.name} Chefs Directly | Tip a Chef`,
    description: `Send a direct tip to chefs cooking in ${data.name}. No app, no middleman. Just a QR code, a phone, and thirty seconds.`,
    openGraph: {
      title:       `Support ${data.name} Chefs | Tip a Chef`,
      description: `Direct chef tipping in ${data.name}. Find a chef, send a tip, leave a message.`,
      url:         `https://tipachef.com/chefs/${params.city}`,
      type:        "website",
    },
    alternates: { canonical: `https://tipachef.com/chefs/${params.city}` },
  };
}
...
export default function CityPage({ params }: { params: { city: string } }) {
  ...
  const jsonLd = {
    ...
    "@type":    "ItemList",
    name:       `Chefs to support in ${data.name}`,
    url:        `https://tipachef.com/chefs/${params.city}`,
      ...
      <main style={{ background: "#0a0908", minHeight: "100vh" }}>
        ...
        <section style={{ paddingTop: "7rem", paddingBottom: "4rem" }}>
          <div className="content-container text-center">
            ...
            <p className="eyebrow mb-4 mt-4">{data.flag} {data.country}</p>
            <h1
                  ...
                  <Link key={chef.slug} href={`/${chef.slug}`} className="group block" style={{ textDecoration: "none" }}>
                    ...
                    <div
                      style={{
                        ...
                        aspectRatio: "3/4",
                        boxShadow: "0 24px 60px rgba(0,0,0,0.6),0 8px 24px rgba(0,0,0,0.4),0 0 0 1.5px rgba(255,255,255,0.07)",
          ...
          <section style={{ paddingBottom: "5rem" }}>
            <div className="content-container text-center">
              <div style={{ maxWidth: "440px", margin: "0 auto", background: "rgba(255,255,255,0.03)", border: "1.5px solid rgba(201,169,110,0.15)", borderRadius: "24px", padding: "3rem 2rem" }}>
                <p style={{ fontSize: "2.5rem", marginBottom: "1rem" }}>{data.flag}</p>
                <p style={{ fontFamily: "Georgia,'Times New Roman',serif", fontStyle: "italic", fontSize: "1.15rem", color: "rgba(250,248,244,0.6)", marginBottom: "2rem", lineHeight: 1.7 }}>
                  No chefs from {data.name} have joined yet. Be the first.
                </p>
                <Link
                  href="/signup"
        ...
        <section style={{ paddingBottom: "6rem", borderTop: "1px solid rgba(255,255,255,0.05)", paddingTop: "4rem" }}>
          <div className="content-container" style={{ maxWidth: "680px" }}>
            ...
            <div style={{ display: "grid", gap: "2rem" }}>
              {[
                { n: "01", title: "Find the QR code", body: `Look for a Tip a Chef QR code at your table, on the menu, or at the pass in ${data.name} restaurants that have joined.` },
        ...
        <section style={{ paddingBottom: "6rem" }}>
          <div className="content-container text-center">
            <p style={{ fontFamily: "-apple-system,system-ui", fontSize: "0.95rem", color: "rgba(250,248,244,0.4)", marginBottom: "1.25rem" }}>
              Chef in {data.name}? Get your free profile.
            </p>
            <Link
              href="/signup"
