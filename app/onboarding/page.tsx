...
export default function Onboarding() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (error || !data) {
          // Chef row missing — could happen if signup didn't finish creating it
  ...
  async function uploadAvatar(file: File) {
    if (!chef) return;
    // Show immediately from local file — never disappears
    ...
    try {
      ...
      if (!upErr) {
        ...
        // Bucket may not exist yet — keep local preview, save will retry via dashboard
  ...
  function goStep3Done() {
    setStep(4);
    // Trigger notification animation after transition
    setTimeout(() => setNotifVisible(true), 400);
    // Save tip_reward in background — ignore errors if column not yet added
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={() => router.replace("/dashboard")} className="text-amber-600 underline text-sm">Go to dashboard</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex">
        ...
        <div className="flex-1 flex items-center justify-center px-8 lg:px-14 py-12">
          ...
          {step === 1 && (
            <div className="w-full max-w-sm">
              ...
              <h1 className="text-gray-900 font-semibold leading-tight mb-2" style={{ fontSize: "clamp(1.7rem,3vw,2.2rem)", letterSpacing: "-0.03em" }}>
                Tell us who you are,<br />Chef.
              </h1>
              <p className="text-gray-400 text-sm mb-8">This appears on your public tip page. Edit any time.</p>
              ...
              <div className="space-y-3">
                ...
                <div className="relative">
                  ...
                  {showRoles && filteredRoles.length > 0 && (
                    <div className="absolute left-0 right-0 top-full mt-1.5 bg-white rounded-2xl border border-gray-100 shadow-xl z-20 py-1.5 max-h-44 overflow-y-auto">
                      {filteredRoles.map(r => (
          ...
          {step === 2 && (
            <div className="w-full max-w-sm">
              ...
              <p className="text-gray-400 text-sm mb-8">
                Profiles with a photo get <span className="font-semibold text-gray-600">3x more tips</span>. You can skip and add later.
              </p>
              ...
              <label className="block cursor-pointer group">
                <div
                  ...
                  style={{ borderColor: avatarUrl ? "#C9A96E" : "#E5E7EB", background: avatarUrl ? "transparent" : "#F9FAFB" }}
                >
                  {avatarUrl ? (
                    <>
                    ...
                    <div className="flex flex-col items-center gap-3 group-hover:scale-105 transition-transform">
                      <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl" style={{ background: "#FEF3E2" }}>📸</div>
          ...
          {step === 3 && (
            <div className="w-full max-w-sm">
              ...
              <p className="text-gray-300 text-xs mt-2 mb-5">Tippers see this after their tip completes. Edit any time in Settings.</p>

              <div className="flex gap-3">
                <button onClick={goStep3Done}
                  className="flex-[0.55] py-3.5 rounded-2xl text-sm font-medium border border-gray-200 text-gray-400 hover:bg-gray-50 transition-all"
                >
                  Skip
                </button>
                <button onClick={goStep3Done}
      ...
      <div
        ...
        style={{ background: "linear-gradient(160deg,#0f0f0f,#1a1208)" }}
      >
